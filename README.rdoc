...
{<h2 id="documentation">Table of Contents</h2>}[#]
...
- {Examples}[#label-Examples]
  - {Controller with CRUD functionality}[#label-Controller+with+CRUD+functionality]
    - {Customize single views}[#label-Customize+single+views]
    - {Adapt general behavior}[#label-Adapt+general+behavior]
    - {Special formatting for selected attributes}[#label-Special+formatting+for+selected+attributes]
    - {Sorting and filtering the index list}[#label-Sorting+and+filtering+the+index+list]
  - {Standard Tables and Forms}[#label-Standard+Tables+and+Forms]
    - {Tables}[#label-Tables]
    - {Forms}[#label-Forms]
  - {Nested Resources}[#label-Nested+Resources]
  - {CRUD controller callbacks}[#label-CRUD+controller+callbacks]
  - {Internationalization (I18N)}[#label-Internationalization]
  - {Example Code}[#label-Example+Code]
- {Generated Files}[#label-Generated+Files]
